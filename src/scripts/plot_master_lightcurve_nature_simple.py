...
matplotlib.rcParams['font.sans-serif'] = ['Verdana','Helvetica']
...
def masterplot_paper(neowise, aavso_b, aavso_v, aavso_i, asassn_v, asassn_g, atlas_c, atlas_o, lcogt_g, lcogt_i, lcogt_r):
 ...
 #   ax.set_xlabel("Epoch [MJD]", fontsize=16)
    ...
    matplotlib.rcParams['mathtext.rm'] = 'Verdana'
    matplotlib.rcParams['mathtext.it'] = 'Verdana:italic'
    matplotlib.rcParams['mathtext.bf'] = 'Verdana'
    matplotlib.rcParams['mathtext.sf'] = 'Verdana'

    ax3.set_ylabel(r'$v$ [$km\ s^{-1}$]', fontsize=16, font='Verdana')
#    ax3.set_ylabel('v [km/s]', fontsize=16)
    ax3.set_xlim(59100,59900) 


    tyb = dict(color='black', weight='bold', fontsize=12)
    ax.text(0.03, 0.9, 'a', ha='right', va='top', transform=ax.transAxes, **tyb)
    ax3.text(0.03, 0.9, 'b', ha='right', va='top', transform=ax3.transAxes, **tyb)

    plt.tight_layout()
    fig.savefig(paths.figures / 'master_lightcurve_nature_simple.pdf')
    plt.close()
...
# (ax, ax2,ax3) = np.ndarray.flatten(axes)

# ax.errorbar(tasg['MJD'],tasg['fnorm'], yerr=tasg['fnormerr'],fmt='.',color='black',markersize=2)

# ax.vlines(xc,-1,2,color='black',alpha=0.1)

# ax.axhline(0,color='black',alpha=0.1,linestyle='dotted')
# ax2.axhline(0,color='black',alpha=0.1,linestyle='dotted')
# ax3.axhline(0,color='black',alpha=0.1,linestyle='dotted')
# ax.axhline(1,color='black',alpha=0.1,linestyle='dotted')
...
# ax3.errorbar(fit_x,vtrans.value,yerr=vtrans_err.value,fmt='wv',mec='black',alpha=0.3)
# ax3.errorbar(fit_x[mh],vtrans.value[mh],yerr=vtrans_err.value[mh],fmt='kv')
...
# ax2.set_ylabel('Gradient of flux [$d^{-1}$]')
# ax3.set_ylabel('$v_{trans}$ [$km\ s^{-1}$]')
# ax.set_xlim(59000,60000) 


# vcirc(1.02*u.Msun,10*u.Mearth, 16*u.au)
# Out[16]: <Quantity 7.52037678 km / s>
